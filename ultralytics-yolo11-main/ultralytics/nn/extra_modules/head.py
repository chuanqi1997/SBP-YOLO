...
__all__ = ['Detect_DyHead', 
           ...
           'Detect_AFPN_P345', 'Detect_AFPN_P345_Custom', 'Detect_AFPN_P2345', 'Detect_AFPN_P2345_Custom', 
           'Detect_Efficient', 'Detect_DESH', 'Detect_Efficient_UIB','Detect_MSCH',
           'DetectAux',
           ...
           'Detect_RSCD', 'Segment_RSCD', 'Pose_RSCD', 'OBB_RSCD', 'Detect_FGIF'
           ]
...
class Detect_MSCH(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, ch=()):
        ...
        self.share_conv  = nn.ModuleList ( nn.Sequential(Conv(hidc, hidc, 3), Conv(hidc, hidc, 3)) for x in ch)
        self.share_conv2 = nn.ModuleList ( nn.Conv2d(hidc, 4 * self.reg_max, 1) for x in ch)
        self.share_conv3 = nn.ModuleList ( nn.Conv2d(hidc, self.nc, 1) for x in ch)
    ...
    def bias_init(self):
        ...
        for a, b, s in zip(m.share_conv2, m.share_conv3, m.stride):  # from
            a.bias.data[:] = 1.0  # box
...
#             return x
#         y = self._inference(x)
#         #return y if self.export else (y, x) # TODO: 这里是原来YOLO11的Forward
...
#     dynamic = False  # force grid reconstruction
#     export = False  # export mode False
#     end2end = False  # end2end
#     max_det = 300  # max_det
#     shape = None
#     format = None
...
class Detect_Efficient(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        # exp630 ＜（＾－＾）＞
        # self.stem = nn.ModuleList(nn.Sequential(Conv(x, x, 1, g=x // 16), Conv(x, x, 1, g=x // 16)) for x in ch) # two 3x3 Group Conv
        # self.stem = nn.ModuleList(nn.Sequential(Partial_conv3(x, 4), Conv(x, x, 1)) for x in ch) # one PConv(CVPR2023), one 1x1 Conv
        self.cv2 = nn.ModuleList(nn.Conv2d(x, 4 * self.reg_max, 1) for x in ch)
        self.cv3 = nn.ModuleList(nn.Conv2d(x, self.nc, 1) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
...
from ultralytics.nn.backbone.mobilenetv4 import UniversalInvertedBottleneckBlock
from ultralytics.nn.extra_modules.block import FasterNeXt
from ultralytics.nn.extra_modules.block import ShuffleNetV2
from ultralytics.nn.modules.conv import GhostConv

class Detect_Efficient_UIB(nn.Module):
    """YOLOv8 Detect Efficient head for detection models with UIB."""
    dynamic = False  # force grid reconstruction
    export = False  # export mode
    shape = None
    anchors = torch.empty(0)  # init
    ...
    def __init__(self, nc=80, ch=()):
        ...
        #         Partial_conv3(x, 4)
        #         ) for x in ch
        #     ) # one PConv(CVPR2023), one 1x1 Conv

        
        # self.stem = nn.ModuleList(nn.Sequential(Partial_conv3(x, 4), Conv(x, x, 1)) for x in ch) # one PConv(CVPR2023), one 1x1 Conv
        # self.stem = FasterNeXt(x, x)
        
        # 使用深度可分离卷积优化
        # self.stem = nn.ModuleList(
        #     nn.Sequential(
        #         Conv(x, x, 3, g=x),  # Depthwise Conv
        #         Conv(x, x, 1)        # Pointwise Conv
        #     ) for x in ch
        # )
        ...
        #         ShuffleNetV2(x, x, 1), 
        #         ShuffleNetV2(x, x, 1)
        #         ) for x in ch) # two 3x3 Group Conv
        ...
        #19_YOLOv8-p2-FastNeXt-Head summary: 307 layers, 1,712,232 parameters, 1,712,216 gradients, 4.7 GFLOPs
        # 效果不太好
        # self.stem = nn.ModuleList(
        #     nn.Sequential
        #     (
        #         Conv(x, 2 * x, 1, 1),#逐点卷积升维
        #         Partial_conv3(2 * x, 4), 
        #         Conv(2 * x, x, 1)
        #     ) for x in ch) # one PConv(CVPR2023), one 1x1 Conv
        
        # 19_YOLOv8-p2-FastNeXt-Head summary: 331 layers, 1,248,392 parameters, 1,248,376 gradients, 3.9 GFLOPs
        # self.stem = nn.ModuleList(nn.Sequential(
        #     GhostConv(x, x, 3, g=x // 16), 
        #     GhostConv(x, x, 3, g=x // 16)
        #     ) for x in ch) # two 3x3 Group Conv


        
        # ✅ 使用UIB替换原始的分组卷积stem 效果不好
        # self.stem = nn.ModuleList([
        ...
        #     #     expand_ratio=2.0,  # 扩展通道到2倍（增强特征表达）
        #     #     # expand_ratio=1.0,  # 扩展通道到2倍（增强特征表达）
        #     #     # use_layer_scale=True,  # 启用层缩放提升训练稳定性
        #     #     # layer_scale_init_value=1e-5,
        #     # ) for x in ch
        # ])

        # 保持原有结构，但输入通道需与UIB输出对齐（UIB的输出通道=x）
        # self.cv2 = nn.ModuleList(nn.Conv2d(x, 4 * self.reg_max, 1) for x in ch)
        # self.cv3 = nn.ModuleList(nn.Conv2d(x, self.nc, 1) for x in ch)
        
             # 优化2：合并cv2和cv3
        self.cv_head = nn.ModuleList(
            nn.Conv2d(x, 4 * self.reg_max + self.nc, 1) for x in ch
        )
    ...
    def forward(self, x):
        shape = x[0].shape  # BCHW
        for i in range(self.nl):
            x[i] = self.stem[i](x[i])  # 使用UIB处理特征
            # x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
            x[i] = self.cv_head[i](x[i])  # 合并计算
        # 后续流程保持不变（如训练/推理）
        if self.training:
            return x
        elif self.dynamic or self.shape != shape:
    ...
    def bias_init(self):
        """Initialize Detect() biases, WARNING: requires stride availability."""
        # Iterate over all detection layers
        for i in range(self.nl):
            cv_head = self.cv_head[i]  # Get the merged head conv layer
            b = cv_head.bias.view(-1, )  # Flatten bias (out_channels, )

            # Split into box and class parts
            b_box = b[:4 * self.reg_max]  # First part is box regression
            b_cls = b[4 * self.reg_max:]  # Second part is classification

            # Initialize box bias to 0 or small values
            b_box.data.zero_()  # or b_box.data.fill_(1.0)
            
            # Initialize class bias using prior probability of object appearance
            # This formula mimics YOLOv5/8's default bias initialization
            n_cls = self.nc
            stride = self.stride[i]
            b_cls.data.fill_(math.log(5 / n_cls / (640 / stride) ** 2))  # cls pred sigmoid(x) ≈ 5 objects / img
    # def bias_init(self):
    #     """Initialize Detect() biases, WARNING: requires stride availability."""
    #     m = self  # self.model[-1]  # Detect() module
    #     for a, b, s in zip(m.cv2, m.cv3, m.stride):  # from
