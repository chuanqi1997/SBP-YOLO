...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           'GSConv', 'GSConvns', 'GSConvE2', 'GSConvE', 'VoVGSCSP', 'VoVGSCSPns', 'VoVGSCSPC', 'C3k2_CloAtt', 'SCConv', 'C3k2_SCConv', 'ScConv', 'C3k2_ScConv',
           ...
           'C3k2_SMPCGLU', 'C3k2_Heat', 'SBA', 
           'WaveletPool', 'WaveletUnPool', 'WaveletDownBlock',
           'DynamicConv',
           'DynamicConvAt',
           'Dynamic_conv2d',
           'DynamicConvolution',
           'GhostConv_V2',
           'LDown','LFEM',
           'FasterNeXt','C3_FasterNeXt','C3k2FasterNet',
           'C3_Ghost','C3_Ghost_CAA','C2f_Ghost','C2Ghost','C2GSBC','C2GSB',
           ...
           'C3k2_DynamicFilter', 'C3k2_TSSA', 'A2C2f_CGLU', 'A2C2f_KAN', 'A2C2f_DFFN', 'HAFB',
           'C2f_FasterNeXt',
           'SKConv',
           'SKUnit',
           'MobileConv',
           'PatchEmbed_FasterNet','BasicStage','PatchMerging_FasterNet',
           'ConvMaxPool',
           'GSConvEE',
            'DSC3k2','CGAF','SNI','HyperACE','DownsampleConv','FullPAD_Tunnel',
           ]
...
class GSConvEE(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.cv2 = Conv(c_, c_, k=3, s=1, g=c_)
        self.cv3 = Conv(c_, c_, k=5, s=1, g=c_)
        self.cv4 = Conv(c_, c_, k=7, s=1, g=c_)
...
class DynamicConv_Single_Onnx(nn.Module):
...
from ultralytics.nn.extra_modules.attention import EMA

class DynamicConvAt(nn.Module):
    default_act = nn.SiLU()  # default activation
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True, num_experts=4):
        super().__init__()
        self.conv = DynamicConv(c1, c2, k, s)
        self.at = EMA(c2)
    def forward(self, x):
        shortcut = x
        x = self.conv(x)
...
class Dynamic_conv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, ratio=0.25, dilation=1, groups=1, bias=True, K=4,temperature=34, init_weight=True):
...
class DynamicConvolution(TempModule):
    def __init__(self,  in_channels, out_channels, kernel_size,
                 stride=1, padding=0, nof_kernels =3 , reduce = 1, dilation=1, groups=1, bias=True):
        """
...
class WaveletDownBlock(nn.Module):
    def __init__(self, c1, c2):
        """小波下采样块：Conv + WaveletPool + 通道压缩"""
        super().__init__()
        self.conv = Conv(c1, c2, k=3, p=1)       # 标准卷积层
        self.pool = WaveletPool()                # 小波池化层
        self.reduce_conv = Conv(4*c2, c2, k=1)   # 通道压缩层（将4c2→c2）
...
class WaveletUpBlock(nn.Module):
    def __init__(self, c1, c2, c_skip):
        """小波上采样块：通道调整 + WaveletUnPool + 特征融合"""
        super().__init__()
        self.up_conv = Conv(c1, 4*c2, k=3, p=1)  # 调整通道到4c2
        self.unpool = WaveletUnPool()             # 小波反池化层
        self.merge_conv = Conv(c2 + c_skip, c2)   # 特征融合卷积

    def forward(self, x, skip):
        x = self.up_conv(x)
        x = self.unpool(x)                       # 输出通道数变为c2
        x = torch.cat([x, skip], dim=1)          # 与跳跃连接特征拼接
        return self.merge_conv(x)


class WaveletCBS(nn.Module):
    """Wavelet Pooling followed by CBS Convolution for downsampling."""
    def __init__(self, c1, c2):
        super().__init__()
        self.wavelet_pool = WaveletPool()
        # 下采样后通道变为4*c1，用1x1卷积调整到c2
        self.conv = Conv(c1*4, c2, k=1)
    
    def forward(self, x):
        return self.conv(self.wavelet_pool(x))

class WaveletUCBS(nn.Module):
    """CBS Convolution expansion followed by Wavelet UnPooling for upsampling."""
    def __init__(self, c1, c2):
        super().__init__()
        # 上采样前扩展通道到4*c2，以便UnPool后得到c2
        self.conv = Conv(c1, c2*4, k=1)
        self.wavelet_unpool = WaveletUnPool()
    
    def forward(self, x):
        return self.wavelet_unpool(self.conv(x))

######################################## WaveletPool end ########################################

######################################## DeformableConvMaxPool end ########################################
from torchvision.ops import DeformConv2d
class DeformableConvMaxPool(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1):
        super(DeformableConvMaxPool, self).__init__()
        ...
        self.deform_conv = DeformConv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, bias=False)
        self.bn = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)
...
class C3k2FasterNet(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3_FasterNeXt(self.c, self.c, 2, shortcut, g) if c3k else FasterNeXt(self.c, self.c, e = e) for _ in range(n))
...
class GhostBottleneck_CAA(nn.Module):
    """Ghost Bottleneck https://github.com/huawei-noah/ghostnet."""

    def __init__(self, c1, c2, k=3, s=1, mlp_ratio = 3):
...
class C3_Ghost_CAA(nn.Module):
    # C3_Ghost Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, extra=2, shortcut=True, g=1, e=0.5, mlp_ratio = 3):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(*(GhostBottleneck_CAA(c_, c_) for _ in range(n)))

    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))


class C2GSBC(nn.Module):
    def __init__(self, c1, c2, n=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c1, 1)
        self.m = nn.Sequential(*(GSBottleneckC(self.c, self.c) for _ in range(n)))
    def forward(self, x):
...
class C2GSB(nn.Module):
    def __init__(self, c1, c2, n=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c1, 1)
        self.m = nn.Sequential(*(GSBottleneck(self.c, self.c) for _ in range(n)))
    def forward(self, x):
        """Processes the input tensor 'x' through a series of PSA blocks and returns the transformed tensor."""
        a, b = self.cv1(x).split((self.c, self.c), dim=1)
        b = self.m(b)
...
class GhostConv_V2(nn.Module):
    ...
    def __init__(self, num_in, num_out, kernel_size=1, stride=1, padding=0, ratio=2, dw_size=3,
                 use_act=True, act_type='silu'):
        super(GhostConv_V2, self).__init__()
...
class LDown(nn.Module):
    def __init__(self, c1, c2):  # ch_in, ch_out
        ...
        self.conv1 = Conv(c1 // 2, self.c)
        self.conv2 = Conv(c1 // 2, self.c)

        self.pool1 = nn.AvgPool2d(2, stride=2)
        self.pool2 = nn.MaxPool2d(3, stride=2, padding=1)

        self.attention = NAMAttention(self.c * 2)
...
class LFEM(nn.Module):
    def __init__(self, c1, c2, n=1, scale=0.5, e=0.5):
        ...
        self.cv3 = RepConv(self.c, self.mid, 3)
        self.m = nn.ModuleList([nn.Sequential(Conv(self.mid, self.mid, 3), Conv(self.mid, self.mid, 3)) for _ in range(n - 1)])
        self.cv4 = Conv(self.mid, self.mid, 1)
...
class ShuffleNetV2(nn.Module):
    ...
    def forward(self, x):
        if self.stride == 1:
            x1, x2 = x.chunk(2, dim=1) # 将输入数据沿着第一个维度分为两份
...
class Down_WT(nn.Module):
    def __init__(self, in_ch, out_ch=1, wave='haar'):
        super(Down_WT, self).__init__()
        self.wt = DWTForward(J=1, mode='zero', wave=wave)
...
class SKUnit(nn.Module):
    def __init__(self, in_features, out_features, stride=1, M=2, G=32, r=16, L=32):
        """ Constructor
        Args:
...
class ConvMaxPool(nn.Module):  
    def __init__(self, c1, c2, kernel_size=3, stride=2, stride_mp = 2):  # ch_in, ch_out  
        super().__init__()  
        self.conv= nn.Sequential(
            nn.Conv2d(c1, c2, kernel_size=kernel_size, stride=stride, padding=1, bias=False),
            nn.BatchNorm2d(c2),
            nn.ReLU(inplace=True),
        )
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=stride_mp, padding=1, dilation=1, ceil_mode=False)
...
class SNI(nn.Module):
    ...
    def __init__(self, c1=0, c2=0, up_f=2):
        ...
        self.alpha = 1/(up_f**2)
        # print(f"SNI __init__ {c1}, {c2}, {up_f}")
    def forward(self, x):
        # print(f"SNI1:{x.shape}")
        x = self.alpha*self.us(x)
        # print(f"SNI2:{x.shape}")
        return x
