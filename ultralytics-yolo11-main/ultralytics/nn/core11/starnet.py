...
class StarNetConvBN(torch.nn.Sequential):
    def __init__(self, in_planes, out_planes, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, with_bn=True):
        super().__init__()
        self.stem = nn.Sequential(ConvBN(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding), nn.ReLU6())
    def forward(self, x):
        return self.stem(x)
...
#         self.m = nn.Sequential(*(Bottleneck_StarsBlock(c_, c_, shortcut,  g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n)))
