...
from ultralytics.nn.backbone.mobilenetv4 import UniversalInvertedBottleneckBlock as UIB

from ultralytics.nn.extra_modules.lsnet import Block as LSBlock

from ultralytics.nn.extra_modules.FDConv_initialversion import FDConv
from ultralytics.nn.extra_modules.DyConvOnnx import DynamicConv as DyConvONNX
from ultralytics.nn.extra_modules.ghostnetv3_pytorch.ghostnetv3 import GhostModule as GhostConv_V3
from ultralytics.nn.extra_modules.DyDSCon import DynamicDSConv
...
DETECT_CLASS = (Detect, Detect_DyHead, 
...
C3K2_CLASS = (C3k2_Faster, C3k2_ODConv, C3k2_Faster_EMA, C3k2_DBB, C3k2_DeepDBB, C3k2_WDBB, C3k2_CloAtt, C3k2_SCConv, C3k2_ScConv, C3k2_EMSC, C3k2_EMSCP, C3k2_KW, C3k2_DySnakeConv, C3k2_DCNv2, C3k2_DCNv3,
              ...
              C3k2_TSSA,
              FasterNeXt,C3_FasterNeXt,C3k2FasterNet,C3_Ghost,C3_Ghost_CAA,C2f_Ghost,C2Ghost,C2GSBC,C2GSB
              )
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    # _, _, gw, _ = d['nc'], d['depth_multiple'], d['width_multiple'], d.get('activation')
    gw = width
    if scales:
        scale = d.get("scale")
        if not scale:
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in ((
            ...
            MANet_Star, PSConv, C2Pola, A2C2f, C2TSSA, A2C2f_CGLU, A2C2f_KAN, A2C2f_DFFN, RepHMS,
            UIB,LSBlock,WaveletDownBlock,DeformableConvMaxPool,DynamicConv,DynamicConvAt,Dynamic_conv2d,AKConv,
            GhostConv_V2,LDown,LFEM,
            #DCNV4_YOLO,DCNv4_pytorch,DCNv4Block,CKDCNv4,DCNV4_Conv,FDConv,
            ShuffleNetV2,GhostConv_V3,
            SPConv_3x3,
            DynamicConvolution,DyConvONNX,DynamicDSConv,C2f_FasterNeXt,
            SKConv,SKUnit,
            GSConvE2,GSConvE,MobileConv,
            BasicStage, PatchEmbed_FasterNet, PatchMerging_FasterNet, # FasterNet
            ConvMaxPool,GSConvEE,
            DSC3k2,DSConv
        ) + C3K2_CLASS):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            ...
            if m in ((
                ...
                MANet_FasterBlock, MANet_FasterCGLU, MANet_Star, C2Pola, A2C2f, C2TSSA, A2C2f_CGLU, A2C2f_KAN, A2C2f_DFFN
            ) + C3K2_CLASS):
                args.insert(2, n)  # number of repeats
                n = 1
            elif m in [BasicStage]:# FasterNet
                args.pop(1)
            ...
            c2 = m.channel
                   ...
                   DySample, CARAFE, CAA, ELA, CAFM, AFGCAttention, EUCB, EfficientChannelAttention, ContrastDrivenFeatureAggregation, FSA,
                   ShuffleAttention, CensusAttention}:
            c2 = ch[f]
            args = [c2, *args]
            # print(args)
