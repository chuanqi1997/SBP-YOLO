...
def get_pruner(opt, model, example_inputs):
    ...
    from ultralytics.nn.extra_modules.block import VoVGSCSPC
    ...
    for k, m in model.named_modules():
        ...
        # if isinstance(m, GhostConv):
        #     ignored_layers.append(m.cv2)
        # 只剪枝这个好像还有点问题
        if isinstance(m, VoVGSCSPC):
            # ignored_layers.append(m.cv1)
            ignored_layers.append(m.gsb)
...
class DetectionCompressor(BaseTrainer):
    ...
    def model_prune(self, imp, prune, example_inputs):
        ...
        while speed_up < self.args.speed_up:
            ...
            print(f"🚀prune_flops：{prune_flops}")
